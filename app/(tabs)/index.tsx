...
export default function HomeScreen() {
  ...
  const formatMonthText = (date: string) => {
  ...
  return (
    <View className="flex-1 bg-white">
      <View className="pt-12 pb-4 px-4 bg-blue-500">
        <View className="flex-row justify-between items-center mb-3">
          <View className="flex-1">
            <Text className="text-2xl font-bold text-white">가계부</Text>
            {party?.id && (
              <Text className="text-white/80 text-sm mt-1">{party.name}</Text>
            )}
          </View>
          {party?.id && (
            <View className={`px-2 py-1 rounded ${party.userRole === 'host' ? 'bg-blue-600' : 'bg-gray-600'}`}>
        ...
        <View className="bg-white/10 rounded-lg p-3">
          <View className="flex-row justify-between items-center mb-2">
            <Text className="text-white text-sm font-semibold">{formatMonthText(selectedDate)}</Text>
          </View>
      ...
      <ScrollView className="flex-1">
        <View className="p-4">
          <Calendar
            ...
            dayComponent={({ date, marking, state }) => {
              ...
              const textColor = isSelected ? '#ffffff' : state === 'disabled' ? '#d1d5db' : baseColor;
                    ...
                    {marking?.marked && (
                      <View
                        style={{
                          ...
                          height: 6,
                          borderRadius: 3,
                          backgroundColor: isSelected ? '#ffffff' : marking.dotColor || '#3b82f6',
                          alignSelf: 'center',
                          marginTop: 4,
                        }}
                      />
        ...
        <View className="px-4 mb-4">
          <View className="bg-gray-50 rounded-lg p-4 flex-row justify-between">
            <View className="items-center flex-1">
              ...
              <Text className="text-green-600 text-xl font-bold">{totals.income.toLocaleString()}원</Text>
            </View>
            <View className="w-px bg-gray-300" />
            <View className="items-center flex-1">
              <Text className="text-gray-500 text-sm mb-1">지출</Text>
              <Text className="text-red-600 text-xl font-bold">{totals.expense.toLocaleString()}원</Text>
            </View>
            <View className="w-px bg-gray-300" />
            <View className="items-center flex-1">
              <Text className="text-gray-500 text-sm mb-1">잔액</Text>
              <Text className={`text-xl font-bold ${cumulativeBalance >= 0 ? 'text-blue-600' : 'text-red-600'}`}>
        ...
        <View className="px-4 mb-4">
          <Text className="text-lg font-semibold mb-3 text-gray-800">거래 내역</Text>
